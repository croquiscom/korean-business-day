...
function getBusinessDay(_a) {
    ...
    while (days_to_go) {
        ...
        if (!year_holidays_1.KOREAN_HOLIDAYS[ymd.year]) {
            throw new Error("year " + ymd.year + " data not exists");
        }
        if (!year_holidays_1.KOREAN_HOLIDAYS[ymd.year][ymd.month]) {
            throw new Error("month " + ymd.month + " data not exists");
        }
        if (year_holidays_1.KOREAN_HOLIDAYS[ymd.year][ymd.month][ymd.day]) {
            continue;
        }
        days_to_go--;
    }
...
function isHoliday(date) {
    ...
    var day = date.getDay();
    var ymd = getYmdByDate(date);
    var result = year_holidays_1.KOREAN_HOLIDAYS[ymd.year][ymd.month][ymd.day] !== undefined;
